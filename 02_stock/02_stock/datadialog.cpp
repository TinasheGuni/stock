...
DataDialog::DataDialog(QWidget *parent)
    : QDialog(parent), dataGrid(new QGridLayout(this)),
      itemTypeConfectionery(new QRadioButton("Confectionery")),
      itemTypeReadingMaterial(new QRadioButton("ReadingMaterial")),
      itemName(new QLineEdit), itemQuantity(new QSpinBox),
      itemWeight(new QSpinBox), itemFrequency(new QLineEdit),
      process(new QPushButton("Proces Data")) {
  dataGrid->addWidget(new QLabel("Complete the required data"), 0, 0, 1, 2,
                      Qt::AlignHCenter);
  ...
  connect(itemTypeReadingMaterial, &QRadioButton::toggled, this,
          &DataDialog::manageRadioButtons);
  connect(process, &QPushButton::clicked, this,
          &DataDialog::processButtonClicked);
}
